#!/bin/bash
# MXFP4 RTN baseline (no rotation, no norm fusion) PPL evaluation.
# Qwen3-8B / Qwen3-14B, W4A4 MXFP4 (RTN weights), KV cache 16-bit, WikiText-2 PPL.
# Derived from scripts/MXFP4/RTN/llama/parallel/2_eval_ptq.sh. This machine has 2 x RTX 3090, so the models run
...
for MODEL_PATH in "${MODELS[@]}"; do
    ...
    OUTPUT_DIR="${REPO}/logs/MXFP4/RTN/${MODEL_NAME}"
    mkdir -p "$OUTPUT_DIR"
