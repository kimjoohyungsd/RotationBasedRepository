#!/bin/bash
# MXFP4 RTN with RMSNorm-gain fusion into the following linears (no rotation) PPL evaluation.
# Qwen3-8B / Qwen3-14B, W4A4 MXFP4 (RTN weights), KV cache 16-bit, WikiText-2 PPL.
# Derived from scripts/MXFP4/RTN_Norm_Fused/llama/parallel/2_eval_ptq.sh. This machine has 2 x RTX 3090, so the models run
# one after another, each model_parallel-distributed (--distribute) over GPU 0,1.
# Logs: logs/MXFP4/RTN_Norm_Fused/<model_name>/
...
for MODEL_PATH in "${MODELS[@]}"; do
    ...
    OUTPUT_DIR="${REPO}/logs/MXFP4/RTN_Norm_Fused/${MODEL_NAME}"
    mkdir -p "$OUTPUT_DIR"
    ...
    CUDA_VISIBLE_DEVICES=$CUDA_DEVICES $PY ptq_qwen3.py \
        ...
        "${COMMON_ARGS[@]}" \
        --eval_out_path "${OUTPUT_DIR}/log_W4A4_MXFP4_RTN.txt" \
