...
for CONFIG in "${BIT_CONFIGS[@]}"; do
    ...
    for i in "${!MODELS[@]}"; do
        ...
        CUDA_VISIBLE_DEVICES=$GPU_ID python ptq.py \
            ...
            --rotate --online_r2 --wikitext2 --w_rtn --w_clip   > "${TARGET_DIR}/log_W${W_BIT}A${A_BIT}_ALL_Rotations.txt" 2>&1 &
    done
    wait
    ...
    for i in "${!MODELS[@]}"; do
        ...
        CUDA_VISIBLE_DEVICES=$GPU_ID python ptq.py \
            ...
            --w_bits $W_BIT --a_bits $A_BIT --k_bits 16 --v_bits 16 \
            --a_asym --k_asym --v_asym --k_groupsize 128 --v_groupsize 128 \
            --rotate --deactivate_r1 --online_r2 --wikitext2 --w_rtn --w_clip > "${TARGET_DIR}/log_W${W_BIT}A${A_BIT}_NO_R1.txt" 2>&1 &
    done
    wait
done
