...
def evaluator(model, testenc, dev, args):
    ...
    for i in tqdm(range(len(layers)), desc="(Eval) Layers"):
        ...
        if args.capture_layer_io and args.layer_idx == i:
            captured_io = model_utils.capture_layer_io(layer, inps) # 하나의 Decoder layer 단위에서의 input과 output으로 나눠서 hook를 추가한다 {inputs: {q,k,v, o, up,gate, down}} 2) 실제 forward pass를 진행하고 
    ...
    for i in range(nbatches):
        ...
        loss = loss_fct(shift_logits.permute(0, 2, 1), shift_labels) # [batch,hidden_dim,seq_len-1] [batch,seq_len-1] => output [batch,seq_len-1]
        neg_log_likelihood = loss.float().mean(dim=1) # [batch]
